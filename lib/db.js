...
// Importações - Usar importação dinâmica para evitar problemas no build
let Pool;
try {
  // Tentar importar o módulo pg usando require
  const pg = require('pg');
  Pool = pg.Pool;
  console.log('✅ Módulo pg importado com sucesso via require');
} catch (error) {
  console.error('❌ Erro ao importar módulo pg via require:', error.message);
